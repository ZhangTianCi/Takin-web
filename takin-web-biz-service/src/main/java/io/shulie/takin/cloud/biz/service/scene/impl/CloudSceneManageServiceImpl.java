...
public class CloudSceneManageServiceImpl extends AbstractIndicators implements CloudSceneManageService {
        ...
        if (updateReq.isPressure()) {
            ...
            if (Objects.nonNull(sceneId)) {
                ...
                if (CollectionUtils.isNotEmpty(uploadFile)) {
                    ...
                    List<SceneScriptRefOutput> scriptFiles = filesMap.get(FileTypeEnum.SCRIPT.getCode());
                    List<String> scriptPaths = Collections.singletonList(scriptFiles.get(0).getUploadPath());
                    scriptVerityExt.setScriptPaths(reWritePathIfNecessary(scriptPaths, absolutePath));
                    List<SceneScriptRefOutput> csvFiles = filesMap.get(FileTypeEnum.DATA.getCode());
                    if (CollectionUtils.isNotEmpty(csvFiles)) {
                        List<String> csvPaths = csvFiles.stream().map(SceneScriptRefOutput::getUploadPath)
                            .filter(Objects::nonNull).collect(Collectors.toList());
                        scriptVerityExt.setCsvPaths(reWritePathIfNecessary(csvPaths, absolutePath));
                    }
                    List<SceneScriptRefOutput> attachmentFiles = filesMap.get(FileTypeEnum.ATTACHMENT.getCode());
                    if (CollectionUtils.isNotEmpty(attachmentFiles)) {
                        List<String> attachmentPaths = attachmentFiles.stream().map(SceneScriptRefOutput::getUploadPath)
                            .filter(Objects::nonNull).collect(Collectors.toList());
                        scriptVerityExt.setAttachments(reWritePathIfNecessary(attachmentPaths, absolutePath));
                    }
                }
                ...
                if (StringUtils.isNotBlank(scriptPath)) {
                    scriptVerityExt.setScriptPaths(reWritePathIfNecessary(Collections.singletonList(scriptPath), absolutePath));
                }
    ...
    private String reWritePathIfNecessary(List<String> paths, boolean absolutePath) {
        return paths.stream().map(path -> appConfig.reWritePathByNfsRelativePath(path, absolutePath))
